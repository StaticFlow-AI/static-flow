...
| 子系统 | 文件 | 说明 |
|---|---|---|
| Runtime wiring | `deps/llm-access/crates/llm-access/src/runtime.rs` | API 进程打开 journal sink，并把 analytics sink 指向 journal |
| Journal producer | `deps/llm-access/crates/llm-access/src/usage_journal.rs` | API producer 写 active/sealed journal |
| Public usage forwarding | `deps/llm-access/crates/llm-access/src/public.rs` | public usage 页面转发 worker 查询 |
| Query contract | `deps/llm-access/crates/llm-access/src/usage_query.rs` | worker list/detail/chart HTTP contract |

### 13.2 Worker 与 tiered storage

| 子系统 | 文件 | 说明 |
|---|---|---|
| Worker binary | `deps/llm-access/crates/llm-access/src/bin/llm-access-usage-worker.rs` | 打开 tiered DuckDB repository 并启动 HTTP/query/import loop |
| Worker runtime | `deps/llm-access/crates/llm-access/src/usage_worker.rs` | journal consumer、worker status、query router |
| Tiered repository | `deps/llm-access/crates/llm-access-store/src/duckdb.rs` | active/archive/catalog、rollover、查询规划 |
| Core trait | `deps/llm-access/crates/llm-access-core/src/store.rs` | `UsageEventSink`、`UsageAnalyticsStore`、`UsageEventQuery` |
